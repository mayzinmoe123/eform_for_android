...
class Cp11GovAllow extends StatefulWidget {
  const Cp11GovAllow({Key? key}) : super(key: key);

  @override
  State<Cp11GovAllow> createState() => _Cp11GovAllowState();
}

class _Cp11GovAllowState extends State<Cp11GovAllow> {
  int? formId;
  ...
  AppBar applicationBar() {
    return AppBar(
      centerTitle: true,
      title: const Text("ဝန်ကြီးဌာန ခွင့်ပြုမိန့်",
  ...
  Widget front() {
    return (frontFiles.length <= 0)
        ? multipleUploadWidget('သက်ဆိုင်ရာဝန်ကြီးဌာန ခွင့်ပြုမိန့်', true,
            frontFilesError, frontExplorer)
        : imagePreviewWidget(
            'သက်ဆိုင်ရာဝန်ကြီးဌာန ခွင့်ပြုမိန့်', true, frontFiles, frontClear);
  }
