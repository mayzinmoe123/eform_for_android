...
class CForm15Apartment extends StatefulWidget {
  const CForm15Apartment({Key? key}) : super(key: key);

  @override
  State<CForm15Apartment> createState() => _CForm15ApartmentState();
}

class _CForm15ApartmentState extends State<CForm15Apartment> {
  int? formId;
  ...
  final subTitle = const Text(
    "လျှောက်ထားသူ၏ တိုက်ပုံစံဓါတ်ပုံ(BQ)",
    style: TextStyle(
      fontSize: 18,
  ...
  Widget front() {
    return (frontFiles.length <= 0)
        ? multipleUploadWidget(
            'တိုက်ပုံစံဓါတ်ပုံ(BQ)', true, frontFilesError, frontExplorer)
        : imagePreviewWidget(
            'တိုက်ပုံစံဓါတ်ပုံ(BQ)', true, frontFiles, frontClear);
  }
