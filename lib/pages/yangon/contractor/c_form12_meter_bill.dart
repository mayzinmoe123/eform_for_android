...
class CForm12MeterBill extends StatefulWidget {
  const CForm12MeterBill({Key? key}) : super(key: key);

  @override
  State<CForm12MeterBill> createState() => _CForm12MeterBillState();
}

class _CForm12MeterBillState extends State<CForm12MeterBill> {
  int? formId;
  ...
  AppBar applicationBar() {
    return AppBar(
      centerTitle: true,
      title:
          const Text("(မီတာ/ယာယီမီတာ) ချလံ", style: TextStyle(fontSize: 18.0)),
  ...
  Widget front() {
    return (frontFile == null)
        ? uploadWidget('နောက်ဆုံးပေးဆောင်ထားသော (မီတာ/ယာယီမီတာ) ချလံ ', true,
            frontFileError, frontExplorer)
        : previewWidget('နောက်ဆုံးပေးဆောင်ထားသော (မီတာ/ယာယီမီတာ) ချလံ ', true,
  ...
  void goToNextPage() async {
    final result = await Navigator.pushNamed(context, 'ygn_c_form13_farm_land',
