...
class CpForm10YCDCMdy extends StatefulWidget {
  const CpForm10YCDCMdy({Key? key}) : super(key: key);

  @override
  State<CpForm10YCDCMdy> createState() => _CpForm10YCDCMdyState();
}

class _CpForm10YCDCMdyState extends State<CpForm10YCDCMdy> {
  int? formId;
  ...
  AppBar applicationBar() {
    return AppBar(
      centerTitle: true,
      title: const Text("စည်ပင်သာယာလိုင်စင်", style: TextStyle(fontSize: 18.0)),
  ...
  Widget front() {
    return (frontFiles.length <= 0)
        ? multipleUploadWidget(
            'စည်ပင်သာယာလိုင်စင်', true, frontFilesError, frontExplorer)
        : imagePreviewWidget(
            'စည်ပင်သာယာလိုင်စင်', true, frontFiles, frontClear);
  }
